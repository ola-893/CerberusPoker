...
fn test_6_player_showdown_performance() {
    ...
    println!(
        "Average time per 6-player showdown: {:?}",
        avg_time_per_showdown
    );
    println!(
        "Average time per single hand evaluation: {:?}",
        avg_time_per_hand
    );
    ...
    println!(
        "Estimated CU for 6-player showdown: ~{}",
        estimated_cu_for_6_players
    );
    ...
    let results: Vec<(HandRank, Tiebreaker)> =
        hands.iter().map(|hand| evaluate_hand(hand)).collect();
...
fn test_worst_case_scenario() {
    ...
    println!(
        "Still only {:.2}% of default limit",
        (estimated_cu_worst_case as f64 / 200_000.0) * 100.0
    );
...
fn test_single_hand_evaluation_cost() {
    ...
    println!(
        "This is only {:.2}% of default limit",
        (estimated_cu_single as f64 / 200_000.0) * 100.0
    );
...
fn test_comparison_overhead() {
    ...
    // Evaluate all hands
    let mut results: Vec<(HandRank, Tiebreaker)> =
        hands.iter().map(|hand| evaluate_hand(hand)).collect();
