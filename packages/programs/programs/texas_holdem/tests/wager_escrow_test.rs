...
impl TestContext {
    ...
    async fn new(banks_client: &mut BanksClient, payer: &Keypair, num_players: usize) -> Self {
        ...
        let mut transaction =
            Transaction::new_with_payer(&[create_escrow_ix, init_escrow_ix], Some(&payer.pubkey()));
        transaction.sign(
        ...
        for _ in 0..num_players {
            ...
            let mut transaction =
                Transaction::new_with_payer(&[fund_player_ix], Some(&payer.pubkey()));
...
async fn test_escrow_holds_multiple_player_deposits() {
    ...
    for (_, token_account) in &ctx.player_accounts {
        let balance = ctx
            .get_token_balance(&mut banks_client, &token_account.pubkey())
            .await;
        initial_balances.push(balance);
    }

    // Simulate place_bet for each player (direct SPL transfer to escrow)
    for (i, ((player, player_token_account), &bet_amount)) in ctx
        .player_accounts
        .iter()
        .zip(bet_amounts.iter())
        .enumerate()
    {
    ...
    let escrow_balance = ctx
        .get_token_balance(&mut banks_client, &ctx.escrow_account.pubkey())
        .await;
    assert_eq!(
        escrow_balance, expected_total_pot,
    ...
    for (i, ((_, player_token_account), &bet_amount)) in ctx
        .player_accounts
        .iter()
        .zip(bet_amounts.iter())
        .enumerate()
    {
        let current_balance = ctx
            .get_token_balance(&mut banks_client, &player_token_account.pubkey())
            .await;
...
async fn test_winner_receives_full_pot() {
    ...
    let winner_initial_balance = ctx
        .get_token_balance(&mut banks_client, &winner_token_account.pubkey())
        .await;
...
async fn test_atomic_settlement_single_transaction() {
    ...
    assert!(
        result.is_ok(),
        "Atomic settlement transaction should succeed"
    );
    ...
    assert_eq!(
        ...
        "Winner should have received full pot"
    );
...
async fn test_split_pot_scenario() {
    ...
    for (i, &winner_idx) in winner_indices.iter().enumerate() {
        ...
        println!(
            "Winner {} (player {}) received {} USDC+",
            i, winner_idx, payout_per_winner
        );
    }
    ...
    // Verify escrow is empty
    let escrow_final = ctx
        .get_token_balance(&mut banks_client, &ctx.escrow_account.pubkey())
        .await;
    assert_eq!(
        escrow_final, 0,
        "Escrow should be empty after split pot settlement"
    );
    ...
    for &loser_idx in &loser_indices {
        ...
        let loser_balance = ctx
            .get_token_balance(&mut banks_client, &loser_token_account.pubkey())
            .await;
        // Loser should have initial balance minus their bet
        let expected_loser_balance = 1_000_000_000 - bet_amounts[loser_idx];
        assert_eq!(
            loser_balance, expected_loser_balance,
...
async fn test_escrow_handles_large_pot() {
    ...
    let escrow_balance = ctx
        ...
        .await;
    assert_eq!(
        escrow_balance, total_pot,
        "Escrow should correctly hold large pot from 6 players"
    );
    println!(
        "✓ Escrow correctly holds large pot: {} USDC+ from 6 players",
        escrow_balance
    );
