...
pub fn handler(ctx: Context<AdvancePhase>, _game_id: u64) -> Result<()> {
    ...
    let (next_phase, cards_to_reveal_msg) = match table.phase {
        PokerPhase::PreFlop => (
            PokerPhase::Flop,
            "Client should reveal 3 community cards (indices 0, 1, 2)",
        ),
        PokerPhase::Flop => (
            PokerPhase::Turn,
            "Client should reveal 1 community card (index 3)",
        ),
        PokerPhase::Turn => (
            PokerPhase::River,
            "Client should reveal 1 community card (index 4)",
        ),
        PokerPhase::River => (PokerPhase::Showdown, "No community cards to reveal"),
        PokerPhase::Showdown => {
